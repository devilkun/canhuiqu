...
class Pt extends Home
{
    ...
    //行程列表及预订
    public function stroke($id,$user_id){
        $map= ['dp_exhibition_detail.id'=>$id,'dp_exhibition_detail.status'=>'1'];
        $list =IndexModel::getEXlistOne($map);
        $district = ExhibitionModel::district();
        $similar['type'] = $list['type'];
        $similarlist =IndexModel::similarlist($similar,$list['id']);
        //获取展位发布信息
        $pt = IndexModel::getPtListOne($id,$user_id);
        //获取行程信息
        $stroke = IndexModel::getStrokeList($id,$user_id);
        $this->assign(['list'=>$list,'similar'=>$similarlist,'district'=>$district,'pt'=>$pt,'stroke'=>$stroke,'unread_num'=>unread_num]);
        return $this->fetch();
    }
}